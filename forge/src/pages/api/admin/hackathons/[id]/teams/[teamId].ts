...
export const PUT: APIRoute = async ({ request, cookies, params }) => {
  ...
  return proxyRequest('PUT', `${API}/api/hackathons/${params.id}/teams/${params.teamId}`, token, await request.text());
};

export const DELETE: APIRoute = async ({ cookies, params }) => {
  const token = cookies.get('forge_token')?.value;
  if (!token) return new Response(JSON.stringify({ detail: 'Unauthorized' }), { status: 401 });
  return proxyRequest('DELETE', `${API}/api/hackathons/${params.id}/teams/${params.teamId}`, token);
};
